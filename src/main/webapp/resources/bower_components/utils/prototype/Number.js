/*
 * path    : lib/utils/prototype/Number
 * create  :  2014-02
 * author  :  swing
 * Number原型扩展
 */
...
Number.prototype.toFixed = function(len){
	if(isNaN(len) || len==null || len<0){
		return this;
	}
	return Math.round(this * Math.pow(10,len)) / Math.pow(10,len);
}
//不足位数的补 0
Number.prototype.toFixedString = function(len){
	if(isNaN(len) || len==null || len<0){
		return this + '';
	}
	...
	if(i < 0){
		...
		var s = val.substring(i+1, val.length);
		zeroLen = len - s.length;
	}
