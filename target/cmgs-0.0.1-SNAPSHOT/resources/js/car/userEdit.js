define(function (require) {
    ...
    myApp.controller("mainCtrl", ["$scope", "$http", "$timeout", "CommonList", 
                                  function ($scope, $http, $timeout, CommonList) {

    	$scope.saveData = {};
    	
        $scope.init = function(id) {
        	if (id != null) {
        		$http.get(angular.path + "/car/user/find?id="+id)
        ...
        $scope.save = function() {
        	$http.post(angular.path + "/car/user/save",$scope.saveData)
		 	.success(function(response){
		 		response = jQuery.parseJSON(response);
				if (response.success) {
					alert('保存成功');
					window.location.href = angular.path + "/car/user/list";
				}
		 	});
        }
